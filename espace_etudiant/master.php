<?php
    ...
    if (isset($_POST["submit"])) {  $_SESSION['choix_form'] = $_POST;
        ...
        $selected_masters=$_POST["cycles"];
        foreach ($selected_masters as $master) {
            list($id_filiere , $nom_filiere)=explode('=>',$master);
...
<body>
    ...
    <div class="container">
        <div class="box">
            ...
            <form action="master.php" method="post" enctype="multipart/form-data" onsubmit="return validateCheckboxes()">

                    <label for="note_s6" class="choix_text">Selectionner un Master :</label><br>
                    <?php
        
                        $query_master= $db->prepare('SELECT * FROM filiere WHERE nom_orientation=:orientation');
                        $orientation ="master";
                        $query_master -> bindParam("orientation",$orientation);
                        $query_master ->execute();
                        $rows_master=$query_master->fetchall();
