...
session_start();
    if(isset($_POST["submit"])){
        ...
        $query = $db -> prepare('SELECT * FROM sudo WHERE email=:email');
        $query -> bindparam(":email",$email);
        $query -> execute();
        $row = $query -> fetch();
            if($row && password_verify($password,trim($row["password"]))){
                    //echo "mrhba khoya sudo";
                    $_SESSION["email"]=$email;
                    header("location:affichage_filiere_sudo.php");  
            }
            else{
    ...
    <div class="container">
        <div class="box">
            ...
            <div class="form_container">
                <form action="login_sudo.php" method="post">
                    <div class="info">
                        <div class="email">
